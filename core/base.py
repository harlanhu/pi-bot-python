...
from lib.enums import DevicesIdEnums, FunctionIdEnums
...
class Bot:
    ...
    def smoke_detection(self):
        smoke_detection_function = SmokeDetectionFunction(FunctionIdEnums.SMOKE_DETECTION,
                                                          self.device_manager.get_device(DevicesIdEnums.DEFAULT_BUZZER),
                                                          self.device_manager.get_device(DevicesIdEnums.DEFAULT_SMOG))
        self.function_manager.register(smoke_detection_function.thread_id, smoke_detection_function)

    def body_detection(self):
        body_detection_function = BodyDetectionFunction(FunctionIdEnums.BODY_DETECTION,
                                                        self.device_manager.get_device(DevicesIdEnums.DEFAULT_BODY_INFRARED_SENSOR),
                                                        self.device_manager.get_device(DevicesIdEnums.DEFAULT_BUZZER))
        self.function_manager.register(body_detection_function.thread_id, body_detection_function)

    def thermometer_detection(self):
        thermometer_detection = ThermometerFunction(FunctionIdEnums.THERMOMETER_DETECTION,
                                                    self.device_manager.get_device(DevicesIdEnums.DEFAULT_THERMOMETER))
        self.function_manager.register(thermometer_detection.thread_id, thermometer_detection)

    def oled_display_info(self):
        oled_display_function = OledDisplayFunction(FunctionIdEnums.OLED_DISPLAY,
                                                    self.device_manager.get_device(DevicesIdEnums.DEFAULT_OLED_DISPLAY),
                                                    self.device_manager.get_device(DevicesIdEnums.DEFAULT_THERMOMETER))
